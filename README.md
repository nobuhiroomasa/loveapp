...
- 東京・京都・札幌・那覇など全国 15 都市以上の厳選プランを収録（ベストシーズン／予約情報付き）
- 各プランにはエリア・集合場所・アクセス・推奨ドレスコードなど現地で役立つディテールを付与
- 予算帯（¥ライト〜¥プレミアム）の 7 段階と所要時間、ムード、天気など複数条件でスコアリング
...
3. 利用可能なオプションは `python -m src.date_ai.cli --help` でも確認できます。主なパラメータは以下のとおりです。

   - `--budget`: `¥ライト` から `¥プレミアム` までの 7 段階を指定できます。1段階違いのプランも候補に上がり、差分が説明されます。
   - `--list-budgets`: 7 段階の目安（1人あたりの価格帯や説明）を一覧表示して終了します。
   - `--weather`, `--mood`, `--activity`: 希望条件に合わせてフィルタリング。
   - `--max-hours`: 許容できる最大時間。条件内ならスコアが上昇します。

4. JSON 出力には、プラン詳細とともに `budget_band` 情報（表示名・想定価格帯・説明）が含まれます。
...
2. 別途ブラウザを開き、表示された URL（既定では <http://127.0.0.1:8000>）へアクセスします。
   ...
   - フォームでエリアやムードを選択し、「おすすめを表示」を押すと候補がカード表示されます。各カードには集合場所・アクセス・服装アドバイス・対応言語などの追加情報が表示されます。
...
- `src/date_ai/data.py` にプランを追加するだけで候補を増やせます。`ideal_season` や `tips` を埋めると UI に反映されます。
